...
class Lebagol_breadcrumb {
    ...
    private function __construct() {
        ...
        add_filter('lebagol_page_title', [$this, 'hide_title']);
    ...
    public function breadcrumb_register() {
        $labels = [
            'name'               => esc_html__('Breadcrumbs', 'lebagol'),
            'singular_name'      => esc_html__('Breadcrumb', 'lebagol'),
            'menu_name'          => esc_html__('Breadcrumbs', 'lebagol'),
            'name_admin_bar'     => esc_html__('Breadcrumb', 'lebagol'),
            'add_new'            => esc_html__('Add New', 'lebagol'),
            'add_new_item'       => esc_html__('Add New Breadcrumb', 'lebagol'),
            'new_item'           => esc_html__('New Header Footer & Blocks Template', 'lebagol'),
            'edit_item'          => esc_html__('Edit Header Footer & Blocks Template', 'lebagol'),
            'view_item'          => esc_html__('View Header Footer & Blocks Template', 'lebagol'),
            'all_items'          => esc_html__('All Breadcrumb', 'lebagol'),
            'search_items'       => esc_html__('Search Header Footer & Blocks Templates', 'lebagol'),
            'parent_item_colon'  => esc_html__('Parent Header Footer & Blocks Templates:', 'lebagol'),
            'not_found'          => esc_html__('No Header Footer & Blocks Templates found.', 'lebagol'),
            'not_found_in_trash' => esc_html__('No Header Footer & Blocks Templates found in Trash.', 'lebagol'),
        ];
        ...
        register_post_type('lebagol-breadcrumb', $args);
    }
    ...
    public function register_admin_menu() {
        add_submenu_page(
            'themes.php',
            esc_html__('Breadcrumbs', 'lebagol'),
            esc_html__('Breadcrumbs', 'lebagol'),
            'edit_pages',
            'edit.php?post_type=lebagol-breadcrumb'
        );
    }
    ...
    function ehf_register_metabox() {
        add_meta_box(
            'ehf-meta-box',
            esc_html__('Breadcrumb Options', 'lebagol'),
            [
                $this,
                'efh_metabox_render',
            ],
        ...
        <table class="hfe-options-table widefat">
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('Display On', 'lebagol'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
                       title="<?php echo esc_attr__('Add locations for where this template should appear.', 'lebagol'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    ...
                    HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Display Rules', 'lebagol'),
                            'value'          => '[{"type":"basic-global","specific":null}]',
                            'tags'           => 'site,enable,target,pages',
                            'rule_type'      => 'display',
                            'add_rule_label' => esc_html__('Add Display Rule', 'lebagol'),
                        ],
                        $include_locations
                    );
                    ?>
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('Do Not Display On', 'lebagol'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
                       title="<?php echo esc_attr__('Add locations for where this template should not appear.', 'lebagol'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    <?php
                    HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Exclude On', 'lebagol'),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__('Add Exclusion Rule', 'lebagol'),
                            'rule_type'      => 'exclude',
                        ],
                        $exclude_locations
                    );
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('User Roles', 'lebagol'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__('Display custom template based on user role.', 'lebagol'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    <?php
                    HFE\Lib\Astra_Target_Rules_Fields::target_user_role_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Users', 'lebagol'),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__('Add User Rule', 'lebagol'),
                        ],
                        $users
                    );
                    ?>
    ...
    public static function get_template_id() {
        ...
        $hfe_templates = Astra_Target_Rules_Fields::get_instance()->get_posts_by_conditions('lebagol-breadcrumb', $option);

        foreach ($hfe_templates as $template) {
            return apply_filters('lebagol_breadcrumb_id', $template['id']);
        }
    ...
    public function block_template_frontend() {
        if (is_singular('lebagol-breadcrumb') && !current_user_can('edit_posts')) {
    ...
    function load_canvas_template($single_template) {
        ...
        if ('lebagol-breadcrumb' == $post->post_type) {
