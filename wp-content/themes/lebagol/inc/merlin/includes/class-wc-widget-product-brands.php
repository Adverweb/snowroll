 ...
 * @extends WC_Widget
 */
class Lebagol_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'lebagol');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'lebagol');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'lebagol'),
				'label' => esc_html__('Title', 'lebagol'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'lebagol'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'lebagol'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'lebagol'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'lebagol'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'lebagol'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'lebagol'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'lebagol'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Lebagol_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'lebagol');
			...
			echo '<ul class="product-brands" id="lebagol-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Lebagol_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'lebagol'),
		'singular_name'              => esc_html__('Brands', 'lebagol'),
		'menu_name'                  => esc_html__('Brands', 'lebagol'),
		'all_items'                  => esc_html__('All Brands', 'lebagol'),
		'parent_item'                => esc_html__('Parent Brand', 'lebagol'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'lebagol'),
		'new_item_name'              => esc_html__('New Brand Name', 'lebagol'),
		'add_new_item'               => esc_html__('Add New Brands', 'lebagol'),
		'edit_item'                  => esc_html__('Edit Brand', 'lebagol'),
		'update_item'                => esc_html__('Update Brand', 'lebagol'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'lebagol'),
		'search_items'               => esc_html__('Search Brands', 'lebagol'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'lebagol'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'lebagol'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
