...
<div class="woocommerce-pagination-wrap">
    <?php
    if ($total >= 1) {
        $pagination = lebagol_get_theme_option('woocommerce_shop_pagination', 'default');
        $pagination_text = esc_html__('Load more','lebagol');
        if ('infinit' == $pagination) :
            $pagination_text = esc_html__('Loading...','lebagol');
            wp_enqueue_script('lebagol-waypoints');
        endif;
                ...
                echo paginate_links(
                    apply_filters(
                        ...
                        array( // WPCS: XSS ok.
                               ...
                               'total'     => $total,
                               'prev_text' => esc_html__('Prev', 'lebagol'),
                               'next_text' => esc_html__('Next', 'lebagol'),
                               'type'      => 'list',
                               'end_size'  => 3,
                               'mid_size'  => 3
                        )
